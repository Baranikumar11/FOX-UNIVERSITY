...
export default function Contactget() {
    ...
    useEffect(() => {
        axios.get("https://6888a2d1adf0e59551bac2a4.mockapi.io/hardik")
            .then(res => setUserData(res.data));
    }, []);

    const Delete=(id)=>{  
        if(window.confirm("Are you sure you want to delete this contact?")){
            axios.delete(`https://6888a2d1adf0e59551bac2a4.mockapi.io/hardik/${id}`)
            .then(() => {
                        ...
                        <tbody>
                            {userData.map((user, index) => (
                                <tr key={user.id} style={{ textAlign: 'left' }}>
                                    ...
                                    <td style={{ border: '2px solid black' }}><input type='button' className='btn btn-danger' onClick={()=> Delete(user.id)} value='Delete'></input></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
